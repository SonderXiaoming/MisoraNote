...
class ActionHandler {
  ...
  String getBuffText(double value, {String? valueText, double? actionValue}) {
    final typeValue = value % 1000 ~/ 10;
    final buffText = value == 1
        ? t.skill_hp_max
        : BuffType.getByType(typeValue).getName(t);
    String changeDesc =
        {
          BuffType.criticalDamageTake.value,
          BuffType.damageTake.value,
          BuffType.physicalDamageTake.value,
          BuffType.magicDamageTake.value,
        }.contains(typeValue)
        ? (value % 10 == 0 ? t.skill_reduce : t.skill_increase)
        : (value % 10 == 0 ? t.skill_increase : t.skill_reduce);
    if (valueText != null) {
      changeDesc += " $valueText";
    }
    if (value ~/ 10000 == 1) {
      // 作用
      changeDesc += t.skill_fixed;
    }
    if (actionValue != null && actionValue == 2) {
      changeDesc += t.skill_cannot_dispel;
    }
  ...
  String getTarget() {
    ...
    final depend = (dependId != 0
        ? t.skill_depend_action(dependId % 100)
        : t.none);
  ...
  String getPercent() {
    if (action.actionType == SkillActionType.aura.value ||
      ...
      if (action.actionValue1 == 2 ||
          [
            BuffType.physicalCriticalDamage.value,
            BuffType.magicCriticalDamage.value,
            BuffType.criticalDamageTake.value,
            BuffType.damageTake.value,
            BuffType.physicalDamageTake.value,
            BuffType.magicDamageTake.value,
            BuffType.physicalDamage.value,
            BuffType.magicDamage.value,
          ].contains(action.actionDetail1 ~/ 10)) {
        return "%";
      } else {
        return "";
      }
    } else if (action.actionType == SkillActionType.auraField.value) {
  ...
  String getStatus() {
    return {
          ...
          1700: t.skill_status_1700(getBuffText(action.actionValue3)),
  ...
  String damage() {
    ...
    final adaptive = action.actionDetail2 == 1
        ? t.skill_adaptive_lower_defense
        : "";
  ...
  String move() {
    final directionText = (action.actionValue1 > 0
        ? t.skill_forward
        : t.skill_backward);
    final positionText = (action.actionValue1 > 0
        ? t.skill_ahead
        : t.skill_rear);
  ...
  String speed() {
    ...
    if ([1, 2].contains(action.actionDetail1)) {
      if (action.actionType ==
          SkillActionType.superimposeChangeActionSpeed.value) {
        final type = action.actionDetail1 == 1
            ? t.skill_reduce
            : t.skill_increase;
  ...
  String dot() {
    tag = DotType.get(action.actionDetail1).getName(t);
    final value = getValueText(
      1,
      action.actionValue1,
      action.actionValue2,
  ...
  String dotUp() {
    final effectTypeList = action.actionValue3 == -1
        ? []
        : [
            action.actionValue3,
            action.actionValue4,
    ...
    String effectName = effectTypeList
        ...
        .whereType<String>()
        .join("、");
    effectName = effectName.isNotEmpty ? "『$effectName』" : "";
    final multiple = 1 + action.actionValue1 / 100;
    final limit = t.skill_action_damage_limit_int(action.actionValue7.toInt());
    return t.skill_action_type_desc_110(
      getTarget(),
      effectName,
      multiple.toString(),
      limit,
